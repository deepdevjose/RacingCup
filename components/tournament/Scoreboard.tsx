...
export function Scoreboard({ eventId, categoryId }: ScoreboardProps) {
    ...
    const groupedStats = stats.reduce<Record<string, TeamStats[]>>((acc, stat) => {
        const level = stat.team?.educationLevel || 'General'
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            {Object.entries(groupedStats).map(([level, levelStats]) => (
                <div key={level}>
                    {Object.keys(groupedStats).length > 1 && (
                        <h4 style={{ fontSize: '1rem', color: '#94a3b8', marginBottom: '1rem', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '0.5rem' }}>
                            {level}
                        </h4>
                    )}
            ...
            <style jsx>{`
                tr { transform: translateX(0); transition: transform 0.2s ease; }
